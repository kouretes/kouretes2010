...
using std::endl;
using std::string;
RobotController::RobotController(AL::ALPtr<AL::ALBroker> pBroker,MessageQueue *mq, const std::string &configuration_file) : Thread(false),Publisher("RobotController")
{
    ...
    try {
            memory = new AL::ALMemoryProxy(pBroker);//->getMemoryProxy();
    ...
    cout << "end of  init robot controller" << endl;
}
