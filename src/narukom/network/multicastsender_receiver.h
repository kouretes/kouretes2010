...
class MulticastSender {
	public:
		MulticastSender(NetworkChannel* owner,
				boost::asio::io_service* io_service,
				const boost::asio::ip::address* multicast_address,
				boost::asio::ip::udp::socket* sock, short multicast_port) :
			owner(owner), endpoint_(*multicast_address, multicast_port),
					timer_(*io_service), multicast_port(multicast_port) {
			if (sock == 0) {
		...
		void async_send(const std::string& msg, unsigned int num_of_packet,
			...
			timer_.async_wait(boost::bind(&MulticastSender::handle_timeout,
					this, boost::asio::placeholders::error, num_of_packet,
					message_num));
		}

		void async_send(const char* msg, unsigned int size,
				unsigned int num_of_packet, unsigned int message_num,
				boost::asio::ip::udp::endpoint* endpoint = 0) {
			if (endpoint == 0)
				socket_->async_send_to(boost::asio::buffer(msg, size),
						endpoint_, boost::bind(
								&MulticastSender::handle_send_to, this,
								boost::asio::placeholders::error,
								num_of_packet, message_num));
			else
				socket_->async_send_to(boost::asio::buffer(msg, size),
		...
		void sync_send(const std::string& msg,
				boost::asio::ip::udp::endpoint* endpoint = 0) {
		...
		void sync_send(const char* msg, unsigned int size,
				boost::asio::ip::udp::endpoint* endpoint = 0) {
			if (endpoint == 0)
		...
		void handle_send_to(const boost::system::error_code& error,
			...
			if (!error)
				...
				std::cout << "MulticastSender: " << error.message()
						<< std::endl;
	...
	public:
		MulticasReceiver(NetworkChannel* owner,
			...
			socket_->set_option(boost::asio::ip::multicast::join_group(
					*multicast_address));
			socket_->set_option(boost::asio::ip::multicast::enable_loopback(
					false));

		}
		void sync_receive(char* data, unsigned int sz,
				boost::asio::ip::udp::endpoint* endpoint) {
			socket_->receive_from(boost::asio::buffer(data, sz),
					sender_endpoint_);
		...
		void expire_at() {
			timer_.expires_from_now(boost::posix_time::millisec(50));
			timer_.async_wait(boost::bind(&MulticasReceiver::handle_timeout,
		...
		void async_receive() {
			...
			socket_->async_receive_from(boost::asio::buffer(data_, max_length),
					sender_endpoint_, boost::bind(
							&MulticasReceiver::handle_receive_from, this,
							boost::asio::placeholders::error,
